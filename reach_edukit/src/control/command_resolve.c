...
int32_t init_server_info(server_info *pinfo)
{
	...
	pinfo->LanAddr = inet_addr("192.168.4.45");
	...
	pinfo->WanAddr = inet_addr("192.168.4.86");
	pinfo->WanGateWay = inet_addr("192.168.4.254");
	pinfo->WanNetmask = inet_addr("255.255.255.0");
	r_memset(pinfo->WanMac, 0, 24);
	r_strcpy((int8_t *)pinfo->WanMac, (const int8_t *)"00:22:11:33:55:44");
...
//	pinfo->EncIP = inet_addr("192.168.0.101");
	...
	if(index == 1){
		r_strcpy(pinfo->EncIP, (const int8_t *)"169.254.0.1");
	}
	else if(index == 2){
		r_strcpy(pinfo->EncIP, (const int8_t *)"169.254.0.2");
	}
	else if(index == 3){
		r_strcpy(pinfo->EncIP, (const int8_t *)"169.254.0.30");
	}
	...
	init_quality_info(&pinfo->SD_QuaInfo);
                                        ...
                                        int8_t *send_buf, int32_t msgcode, int8_t *ret_buf, int32_t *ret_len)
{
	...
	if((STATUS_SUCCESS == return_code) && (user_index >= 0))
	{
		...
		system("cp /opt/logo.png /usr/local/reach/dvr_rdk/ti816x/logo_0.png");
	}
	else
	{
		if(user_index >= 0)
                              ...
                              int32_t msgcode)
{
	...
	if(update.Opereate == 0)
	{
		...
		else
		{
			...
			if(ret == 0)
			{
				...
				if(ret != SERVER_VERIFYFILE_FAILED)
				{
					if(ret == SERVER_RET_OK)
					{
						...
						return_code = Dm6467_Upgrade((int8_t *)DM6467_UPDATE_PACKET);
						if(ret == SERVER_RET_OK)
						{
							return_code = STATUS_SUCCESS;
						}
			...
			{
				ret = Dm6467_Upgrade((int8_t*)CONTROL_UPDATE_FILE);
	...
	while(pnode_recv)
	{
		...
		else
		{
			...
			if(r_strcmp(file, (const int8_t *)"/") != 0)
			{
				...
				if(0==r_system((const int8_t *)cmd)){
					/* 删除成功 */
					r_memset(pbuf, 0, XML_VALUE_MAX_LENGTH);
...
//		report_record_status_log(penv, puser, pres, recv_buf);
		...
		if(STATUS_SUCCESS == return_code)
		{
				...
				/* TODO: 广播 */
			{
				...
				if(OPERATION_ONLY_RETURN == ret_type)
					/* 返回 */
				{
					user_index = -1;
					if(RecServer == puser->platform){
...
//	zlog_debug(OPELOG, "roomid = %d\n", roomid);
	...
	/* 编码器1状态 */
	r_memset(pbuf, 0, XML_VALUE_MAX_LENGTH);
	pnode = get_children_node(proom_node, MSG_ROOM_ENC1_STATUS_KEY);
	ret = get_current_node_value((char *)pbuf, XML_VALUE_MAX_LENGTH, pxml->pdoc, pnode);
	if(ret < 0 )
	{
		zlog_error(DBGLOG, "---[room_heart_report_resolve_recv_buf], %s not found!\n", MSG_ROOM_ENC1_STATUS_KEY);
	}
	else
	{
		proom_info_new->EncInfo[0].Status = atoi((const char *)pbuf);
	}

	/* 编码器2状态 */
	r_memset(pbuf, 0, XML_VALUE_MAX_LENGTH);
	pnode = get_children_node(proom_node, MSG_ROOM_ENC2_STATUS_KEY);
	ret = get_current_node_value((char *)pbuf, XML_VALUE_MAX_LENGTH, pxml->pdoc, pnode);
	if(ret < 0 )
	{
		zlog_error(DBGLOG, "---[room_heart_report_resolve_recv_buf], %s found!\n", MSG_ROOM_ENC2_STATUS_KEY);
	}
	else
	{
		proom_info_new->EncInfo[1].Status = atoi((const char *)pbuf);
	}

	/* 编码器3状态 */
	r_memset(pbuf, 0, XML_VALUE_MAX_LENGTH);
	pnode = get_children_node(proom_node, MSG_ROOM_ENC3_STATUS_KEY);
	ret = get_current_node_value((char *)pbuf, XML_VALUE_MAX_LENGTH, pxml->pdoc, pnode);
	if(ret < 0 )
	{
		zlog_error(DBGLOG, "---[room_heart_report_resolve_recv_buf], %s not found!\n", MSG_ROOM_ENC4_STATUS_KEY);
	}
	else
	{
		proom_info_new->EncInfo[2].Status = atoi((const char *)pbuf);
	}
                                   ...
                                   int8_t *send_buf, int32_t msgcode, int8_t *ret_buf, int32_t *ret_len)
{
	...
	for(index = 0; index < array.room_num; index++)
	{
		...
		if(NULL == pforobj)
		{
			zlog_error(DBGLOG, "---[picture_synthesis_req_process] failed, [find_forward_obj] error!\n");
			return_code = STATUS_FAILED;
			continue;
		}
		...
		if(NULL == pforobj->pcmd.forward_process)
		{
			zlog_error(DBGLOG, "---[picture_synthesis_req_process] failed, [pcmd.forward_process] error!\n");
			return_code = STATUS_FAILED;
			continue;
		}
		...
		if(ret < 0)
		{
			zlog_error(DBGLOG, "---[picture_synthesis_req_process] failed, [forward_process] error!\n");
			return_code = STATUS_FAILED;
			continue;
		}
		return_code = STATUS_SUCCESS;
