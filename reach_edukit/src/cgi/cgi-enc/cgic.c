...
char *cgiRemoteUser;
char *cgiRemoteIdent;
char cgiContentTypeData[BUFFER_MIN];
...
static cgiParseResultType cgiParsePostMultipartInput() {
	...
	while (1) {
		char d[BUFFER_MIN];
		char fvalue[BUFFER_MIN];
		char fname[BUFFER_MIN];
		int bodyLength = 0;
		char ffileName[BUFFER_MIN];
		char fcontentType[BUFFER_MIN];
		char attr[BUFFER_MIN];
		char value[BUFFER_MIN];
		fvalue[0] = 0;
		fname[0] = 0;
		ffileName[0] = 0;
		fcontentType[0] = 0;
	...
	int *bodyLengthP, int first)
{
	...
	/* This is large enough, because the buffer into which the
		...
		the attribute name */
	char workingBoundaryData[BUFFER_MIN];
...
cgiFormResultType cgiFormShort(char *name, short *result, short defaultV) {
	cgiFormEntry *e;
	int ch;
	e = cgiFormEntryFindFirst(name);
	if (!e) {
...
void cgiSimpleHeaderCookieSetString(char *name, char *value, int secondsToLive)
{
	/* cgic 2.02: simpler and more widely compatible implementation.
		Thanks to Chunfu Lai. 
	   cgic 2.03: yes, but it didn't work. Reimplemented by
	...
	*/
	...
	gt = gmtime(&then);
	fprintf(cgiOut, 
		"Set-Cookie: %s=%s;  expires=%s, %02d-%s-%04d %02d:%02d:%02d GMT;",
		name, value, 
		days[gt->tm_wday],
		gt->tm_mday,
		months[gt->tm_mon],
...
void cgiHeaderLocation(char *redirectUrl) {
	fprintf(cgiOut, "<SCRIPT LANGUAGE='JavaScript'>location='%s'</SCRIPT> ", redirectUrl);
}

void cgiPageReplace(char* replaceUrl)
{
		fprintf(cgiOut, "<SCRIPT LANGUAGE='JavaScript'>window.location.replace('%s')</SCRIPT> ", replaceUrl);
}
