...
unsigned int mid_clock_ex(void)
{
	unsigned int msec;
	struct timespec tp;
...
int roomgetPreviewPassword(char *password)
{
	...
	ret = appCmdStringParse(ROOM_CONTROL_GET_PREVIEW_PASS, NULL, inlen, password, &outlen, ROOM_TO_CTRL_SERVER_PORT);
	return ret;
}
